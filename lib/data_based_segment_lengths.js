...
function configure(options,cb){
    ...
    var cuser = options.couchdb.auth.username || env.COUCHDB_USER ;
    var cpass = options.couchdb.auth.password || env.COUCHDB_PASS ;
    var chost = options.couchdb.host || env.COUCHDB_HOST || 'localhost';
    var cport = options.couchdb.port || env.COUCHDB_PORT || 5984;
    ...
    var prefix = options.couchdb.collated_prefix || env.COUCH_CACHE_PREFIX || 'imputed%2fcollated'

    var task = _.extend(options,{couchdb:{'couch':couch
                                         ,'cuser':cuser
                                         ,'cpass':cpass
                                         ,'chost':chost
                                         ,'cport':cport
...
function get_relevant_databases(task,next){

    var uri = task.couchdb.couch+'/_all_dbs'
    superagent.get(uri)
    .set('accept','application/json')
    .end(function(err,res){
        var re = /%2f/gi;
        var unescaped_prefix = task.couchdb.prefix.replace(re, "/");

        var matcher = new RegExp('^'+unescaped_prefix)
        task.couchdb.databases =
