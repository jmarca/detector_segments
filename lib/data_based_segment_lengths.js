...
function min_max_detectors(db,next){
    ...
    var view = '_design/collect2/_view/idymdh_array';
    var query = {group:true
                ,reduce:true
                ,group_level:2
                };

    var url = source+'/'+view + '?'+global_utils.toQuery(query)
    var min_max_detector = function(source,view){
        ...
        var wim_800_regex = /wim.8\d\d/;
                               return function(detector,cb){
                                   ...
                                   var url_first =  source+'/'+view + '?'+global_utils.toQuery(startkey);
                                   startkey = _.extend({'startkey':_.flatten([detector.key,{}])},p)
                                   var url_second =  source+'/'+view + '?'+global_utils.toQuery(startkey);
                                   var yr = detector.key[1]

                                   if(detectors[yr] === undefined) detectors[yr]={}
                                   if(detectors[yr][detector.key[0]]===undefined) detectors[yr][detector.key[0]]={'detector':global_utils.convertDetectorIdForSQL(detector.key[0])};
                                   async.parallel([function(pcb){
                                                       request(url_first
                                                              ,function(e,r,b){
                                                                   ...
                                                                   detectors[yr][detector.key[0]].startts = start_date.join('-')+' '+global_utils.pad(start_hr)+':00';
                                                                   return pcb();
                                                               });
                                                   }
                                                  ,function(pcb){
                                                       request(url_second
                                                              ,function(e,r,b){
                                                                   ...
                                                                   detectors[yr][detector.key[0]].stopts = stop_date.join('-')+' '+global_utils.pad(stop_hr)+':00';
                                                                   return pcb();
                                                               });
                                                   }]
                                                 ,function(err){
    ...
    request(url
           ,function(error,response,body){
               ...
               if(detectorlist !== undefined && detectorlist.length > 0){
                   async.whilst(function(){return detectorlist.length > 0;}
                                ,function(callback){
                                    var chunk = detectorlist.splice(0,5);
                                    console.log('next 5 detectors of '+(detectorlist.length+5))
                                    async.forEach(chunk
                                                  ,min_max_detector
                                                  ,callback);
                                }
...
function populate_detectors(err,next){
    // because d00 is on metis at the moment, store these are host db pairs
