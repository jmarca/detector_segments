...
function do_it(err,config){
    ...
    firstloop.awaitAll(function(err,detectors){
        ...
        years.forEach(function(year,idx){
            var grouped_by_freeeway = detectors[idx]
            _.each(grouped_by_freeeway,function(group,freeway_dir){
                // need to make sure events is the same as groups
                secondloop.defer(pg_handler,config,group)

            })

        })
        years.forEach(function(year){
            var stasher = stash_detectors(year)
            stasher(vds)
            stasher(wim)
            var events = generate_event_times(detectors[year])
            secondloop.defer(pg_handler,config,events)
        })
        secondloop.await(function(e){
