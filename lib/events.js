 ...
 *
 * for example:
 * if you have some detectors and say you have records like
 ...
 *   ...
 *
 * then this will make an array of "active" detectors like so:
 *
 ...
 *
 *
 * which when looked at edge-on (by time) gives an array of
 * {time:t1,detectors:[a,c,f]}
 * {time:t2,detectors:[a,c,e,f,g,i,k]}
 * {time:t3,detectors:[a,c,d,e,f,g,h,i,j,k]}
 *   ...
 *
 * So for each time stamp, you have the list of active detectors, so
 * you can determine the effective segment lengths between each
 * detector along the length of the freeway.
 *
...
function generate_event_times(group){
    ...
    // also, be careful to not trip over stupid local time stuff by forcing all parsing to be UTC, and output to be UTC, and to strip off the .000Z at the end of the output string.  This gives a pretty accurate increment the hour by one
